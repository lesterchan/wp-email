 ...
 */
function wp_email_test_source_files() {
 ...
 * use, so "does the plugin still enqueue a library?" would match the comment
 * explaining that it does not, and a test asserting on the raw text fails for
 * the wrong reason -- or worse, passes for one.
 *
 * @param string[] $skip Basenames to leave out.
 * @return string
 */
function wp_email_test_source_code( array $skip = array() ) {
	$code = '';

	foreach ( wp_email_test_source_files() as $file ) {
 ...
 * @return string
 */
function wp_email_test_script_code() {
	$code = '';
 ...
 * @return string
 */
function wp_email_test_read( $relative ) {

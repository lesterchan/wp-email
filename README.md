...
**There is one stylesheet now, and the theme override is gone.** `email-css.css` and `email-css-rtl.css` become a single `css/wp-email.css` written with CSS logical properties, so it lays out correctly in right-to-left languages without a second, mirrored file to keep in step. WP-EMail also no longer loads a copy of the stylesheet out of your theme directory: if you had customised `email-css.css` in your theme, move those rules into your theme's own stylesheet, where they will keep working and can now *add* to the plugin's rules instead of replacing them wholesale. Everything the plugin styles sits under the `.wp-email` class, and the plugin sets no font, no colour and no inline `style` attributes at all.
